# 🤖 Trivy Agent 가이드

> 분산 서버 환경에서 보안 정보를 수집하는 경량 에이전트 시스템

## 📋 목차

1. [아키텍처 개요](#아키텍처-개요)
2. [구성 요소](#구성-요소)
3. [설치 방법](#설치-방법)
4. [동작 원리](#동작-원리)
5. [Collector 확장](#collector-확장)
6. [자산 관리](#자산-관리)
7. [트러블슈팅](#트러블슈팅)

---

## 🏗️ 아키텍처 개요

```
┌─────────────────────────────────────────────────────────────┐
│                    Central Server                            │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │   Nginx     │  │  PHP API    │  │      MySQL          │  │
│  │   :6987     │──│  /api/      │──│  agents             │  │
│  │             │  │  agent.php  │  │  agent_data         │  │
│  └─────────────┘  └─────────────┘  │  agent_commands     │  │
│                                     └─────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
         ▲                    │
         │ HTTPS POST         │ Commands
         │ (register,         │ (scan, collect)
         │  heartbeat,        ▼
         │  report)
┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐
│   Server A      │    │   Server B      │    │   Server C      │
│  ┌───────────┐  │    │  ┌───────────┐  │    │  ┌───────────┐  │
│  │  Agent    │  │    │  │  Agent    │  │    │  │  Agent    │  │
│  ├───────────┤  │    │  ├───────────┤  │    │  ├───────────┤  │
│  │ Collectors│  │    │  │ Collectors│  │    │  │ Collectors│  │
│  │ - trivy   │  │    │  │ - trivy   │  │    │  │ - trivy   │  │
│  │ - system  │  │    │  │ - iptables│  │    │  │ - docker  │  │
│  │ - docker  │  │    │  │ - network │  │    │  │ - custom  │  │
│  └───────────┘  │    │  └───────────┘  │    │  └───────────┘  │
│   [개발]        │    │   [운영]        │    │   [스테이징]    │
└─────────────────┘    └─────────────────┘    └─────────────────┘
```

### 핵심 특징

| 특징 | 설명 |
|------|------|
| **경량** | Alpine 기반 ~50MB 이미지 |
| **확장성** | 플러그인 방식 Collector |
| **자산 관리** | 태그 기반 서버/컨테이너 그룹핑 |
| **실시간** | Docker 이벤트 감시 + 주기적 스캔 |
| **양방향** | 중앙에서 명령 전송 가능 |

---

## 🧩 구성 요소

### 1. Central Server (기존 trivy_Test)

| 컴포넌트 | 경로 | 역할 |
|----------|------|------|
| Agent API | `/api/agent.php` | 에이전트 통신 (등록, 하트비트, 리포트) |
| Admin API | `/api/agent_admin.php` | 관리자용 (목록, 명령 전송) |
| Dashboard | `/agents.php` | 에이전트 관리 UI |
| DB Tables | `agents`, `agent_data`, `agent_commands` | 데이터 저장 |

### 2. Trivy Agent

```
trivy-agent/
├── Dockerfile          # Alpine + Trivy + 도구들
├── agent.sh            # 메인 스크립트
├── collectors/         # 데이터 수집 모듈
│   ├── system.sh       # CPU, 메모리, 디스크
│   ├── processes.sh    # ps aux, 포트
│   ├── docker.sh       # 컨테이너, 이미지
│   ├── iptables.sh     # 방화벽 규칙
│   └── network.sh      # 네트워크 정보
└── scripts/
    └── install.sh      # 원클릭 설치
```

### 3. Collector 종류

| Collector | 수집 데이터 | 용도 |
|-----------|------------|------|
| `trivy` | 컨테이너 취약점 | 보안 스캔 |
| `system` | CPU, 메모리, 디스크, 업타임 | 시스템 모니터링 |
| `processes` | 프로세스 목록, 리스닝 포트 | 프로세스 감시 |
| `docker` | 컨테이너, 이미지, 볼륨 | Docker 자산 |
| `iptables` | INPUT/OUTPUT/FORWARD 규칙 | 방화벽 감사 |
| `network` | 인터페이스, 연결, 라우팅 | 네트워크 감사 |

---

## 🚀 설치 방법

### 방법 1: 로컬 에이전트 (docker-compose에 포함)

현재 docker-compose.yml에 trivy-agent가 포함되어 있습니다.
웹서버와 같은 네트워크에서 `http://trivy-agent:8888`로 통신합니다.

```bash
# docker-compose.yml에 이미 포함됨
# 에이전트 재빌드 시
docker-compose up -d --build trivy-agent
```

### 방법 2: Python 간단 에이전트 (원격 서버용)

기존 서버/컨테이너에 Python만 있으면 바로 설치 가능합니다.

```bash
# 1. 스크립트 다운로드
curl -O http://YOUR_CENTRAL_SERVER:6987/simple_agent.py

# 2. 실행 (백그라운드)
nohup python3 simple_agent.py \
  --url http://YOUR_CENTRAL_SERVER:6987/api/agent.php \
  --token YOUR_AGENT_TOKEN \
  --interval 60 &

# 또는 systemd 서비스로 등록
cat > /etc/systemd/system/trivy-simple-agent.service << EOF
[Unit]
Description=Simple Trivy Agent
After=network.target

[Service]
Type=simple
ExecStart=/usr/bin/python3 /opt/simple_agent.py --url http://CENTRAL:6987/api/agent.php --token TOKEN --interval 60
Restart=always
RestartSec=10

[Install]
WantedBy=multi-user.target
EOF

systemctl daemon-reload
systemctl enable trivy-simple-agent
systemctl start trivy-simple-agent
```

### 방법 3: Docker (원격 서버 전용)

```bash
# 1. 에이전트 이미지 빌드 (Central Server에서)
cd trivy-agent
docker build -t trivy-agent:latest .

# 2. 이미지를 원격 서버로 전송
docker save trivy-agent:latest | ssh user@remote-server docker load

# 3. 원격 서버에서 실행 (MODE=push로 Central에 데이터 전송)
docker run -d \
  --name trivy-agent \
  --restart unless-stopped \
  -v /var/run/docker.sock:/var/run/docker.sock \
  -e CENTRAL_API_URL=http://central-server:6987/api/agent.php \
  -e AGENT_TOKEN=your-secure-token \
  -e MODE=push \
  -e COLLECTORS=trivy,system,docker \
  trivy-agent:latest
```

### 방법 4: Systemd (네이티브)

```bash
# 1. 스크립트 복사
sudo mkdir -p /opt/trivy-agent/collectors
sudo cp agent.sh /opt/trivy-agent/
sudo cp collectors/*.sh /opt/trivy-agent/collectors/
sudo chmod +x /opt/trivy-agent/*.sh /opt/trivy-agent/collectors/*.sh

# 2. Trivy 설치
curl -sfL https://raw.githubusercontent.com/aquasecurity/trivy/main/contrib/install.sh | \
  sudo sh -s -- -b /usr/local/bin v0.29.2

# 3. Systemd 서비스 생성
sudo cat > /etc/systemd/system/trivy-agent.service << EOF
[Unit]
Description=Trivy Security Agent
After=network.target docker.service

[Service]
Type=simple
Environment="CENTRAL_API_URL=http://central-server:6987/api/agent.php"
Environment="AGENT_TOKEN=your-secure-token"
Environment="COLLECTORS=trivy,system,docker"
ExecStart=/opt/trivy-agent/agent.sh
Restart=always

[Install]
WantedBy=multi-user.target
EOF

# 4. 서비스 시작
sudo systemctl daemon-reload
sudo systemctl enable trivy-agent
sudo systemctl start trivy-agent
```

### 환경 변수 설명

| 변수 | 필수 | 기본값 | 설명 |
|------|------|--------|------|
| `CENTRAL_API_URL` | ✅ | - | Central Server API 주소 |
| `AGENT_TOKEN` | ✅ | - | 인증 토큰 |
| `AGENT_ID` | ❌ | hostname-uuid | 에이전트 고유 ID |
| `COLLECTORS` | ❌ | trivy,system,docker | 활성화할 수집기 |
| `HEARTBEAT_INTERVAL` | ❌ | 60 | 하트비트 주기 (초) |
| `SCAN_INTERVAL` | ❌ | 300 | 자동 스캔 주기 (초) |
| `WATCH_DOCKER_EVENTS` | ❌ | true | Docker 이벤트 감시 |

---

## ⚙️ 동작 원리

### 1. 에이전트 시작 흐름

```
┌──────────────┐
│  Agent 시작  │
└──────┬───────┘
       │
       ▼
┌──────────────────┐     ┌─────────────────────┐
│  Central Server  │────▶│  agents 테이블 등록  │
│  /register API   │     │  상태: online       │
└──────────────────┘     └─────────────────────┘
       │
       ├─────────────────────────────────────────┐
       │                                          │
       ▼                                          ▼
┌──────────────────┐                    ┌─────────────────────┐
│  Heartbeat Loop  │                    │  Docker Event Watch │
│  (60초 주기)      │                    │  (실시간)            │
└──────────────────┘                    └─────────────────────┘
       │                                          │
       ▼                                          ▼
┌──────────────────┐                    ┌─────────────────────┐
│  명령 수신/실행   │                    │  새 컨테이너 감지    │
│  (scan, collect) │                    │  자동 Trivy 스캔     │
└──────────────────┘                    └─────────────────────┘
```

### 2. 데이터 흐름

```
                    Agent                           Central Server
                      │                                   │
   ┌──────────────────┴──────────────────┐               │
   │                                      │               │
   ▼                                      ▼               │
┌─────────┐                         ┌─────────┐          │
│Collector│  ─────JSON────────────▶ │ Report  │ ────────▶│
│ 실행    │                         │  API    │          │
└─────────┘                         └─────────┘          │
   │                                                      │
   │ system.sh → {"cpu": 45, "memory": 78, ...}          │
   │ docker.sh → {"containers": [...], ...}              │
   │ trivy scan → {"vulnerabilities": [...]}       ┌─────┴─────┐
   │                                               │   MySQL   │
   └──────────────────────────────────────────────▶│ agent_data│
                                                   └───────────┘
```

### 3. 명령 처리 흐름

```
   Admin Dashboard                 Central Server                    Agent
        │                               │                              │
        │  "전체 스캔" 버튼 클릭          │                              │
        │──────────────────────────────▶│                              │
        │                               │                              │
        │                    ┌──────────┴──────────┐                   │
        │                    │ agent_commands 저장  │                   │
        │                    │ status: pending     │                   │
        │                    └──────────┬──────────┘                   │
        │                               │                              │
        │                               │◀────── Heartbeat (60초) ──────│
        │                               │                              │
        │                    ┌──────────┴──────────┐                   │
        │                    │ 대기 명령 반환       │                   │
        │                    │ {"command": "scan"} │                   │
        │                    └──────────┬──────────┘                   │
        │                               │                              │
        │                               │───────────────────────────▶ │
        │                               │                    ┌────────┴────────┐
        │                               │                    │  명령 실행       │
        │                               │                    │  Trivy 스캔     │
        │                               │                    └────────┬────────┘
        │                               │                              │
        │                               │◀────── 결과 리포트 ───────────│
        │                               │                              │
        │                    ┌──────────┴──────────┐                   │
        │                    │ scan_history 저장   │                   │
        │                    │ agent_id 포함       │                   │
        │                    └─────────────────────┘                   │
```

---

## 🔌 Collector 확장

### 새 Collector 만들기

```bash
#!/bin/bash
# collectors/my_custom.sh
# 반드시 유효한 JSON을 stdout으로 출력

cat <<EOF
{
    "custom_metric_1": $(some_command | wc -l),
    "custom_metric_2": "$(another_command)",
    "collected_at": "$(date -Iseconds)"
}
EOF
```

### Collector 활성화

```bash
# 방법 1: 환경 변수
-e COLLECTORS=trivy,system,docker,my_custom

# 방법 2: 수동 실행
docker exec trivy-agent /opt/agent/collectors/my_custom.sh
```

### 예제: SSL 인증서 만료 체크

```bash
#!/bin/bash
# collectors/ssl_certs.sh

check_cert() {
    local host=$1
    local expiry=$(echo | openssl s_client -servername $host -connect $host:443 2>/dev/null | \
        openssl x509 -noout -enddate 2>/dev/null | cut -d= -f2)
    local days_left=$(( ($(date -d "$expiry" +%s) - $(date +%s)) / 86400 ))
    echo "\"$host\": {\"expiry\": \"$expiry\", \"days_left\": $days_left}"
}

cat <<EOF
{
    "ssl_certificates": {
        $(check_cert "google.com"),
        $(check_cert "github.com")
    },
    "collected_at": "$(date -Iseconds)"
}
EOF
```

---

## 🏷️ 자산 관리

### 자산 그룹/태그 시스템

에이전트와 컨테이너를 **태그**로 분류하여 관리할 수 있습니다.

| 태그 유형 | 예시 | 용도 |
|----------|------|------|
| **환경** | `dev`, `staging`, `prod` | 환경별 분리 |
| **팀** | `backend`, `frontend`, `infra` | 팀별 관리 |
| **서비스** | `api`, `web`, `db` | 서비스 유형 |
| **중요도** | `critical`, `normal`, `low` | 우선순위 |

### 사용 예시

```bash
# 에이전트 등록 시 태그 지정
docker run -d \
  --name trivy-agent \
  -e AGENT_TAGS="prod,backend,critical" \
  ...

# 또는 Dashboard에서 수동 태그 부여
```

### Grafana 필터링

```promql
# 프로덕션 환경만 조회
trivy_vulnerabilities_total{asset_group="prod"}

# 개발+스테이징 환경 조회
trivy_vulnerabilities_total{asset_group=~"dev|staging"}
```

---

## 🔧 트러블슈팅

### 에이전트가 등록되지 않음

```bash
# 1. 네트워크 확인
curl -v http://central-server:6987/api/agent.php?action=list

# 2. 토큰 확인
docker logs trivy-agent | grep -i "auth\|token\|401"

# 3. 방화벽 확인
iptables -L -n | grep 6987
```

### 스캔 결과가 전송되지 않음

```bash
# 1. Trivy 실행 확인
docker exec trivy-agent trivy image alpine:latest

# 2. API 테스트
docker exec trivy-agent curl -X POST \
  -H "X-Agent-Token: $AGENT_TOKEN" \
  -d '{"data_type":"test","data":"{}"}' \
  "$CENTRAL_API_URL?action=report"
```

### Docker 이벤트 감시 안됨

```bash
# docker.sock 마운트 확인
docker inspect trivy-agent | grep -A5 Mounts

# 권한 확인
docker exec trivy-agent ls -la /var/run/docker.sock
```

---

## 📊 모니터링

### 에이전트 상태 확인

| 상태 | 의미 | 조치 |
|------|------|------|
| 🟢 Online | 정상 동작 | - |
| 🟡 Error | 마지막 작업 실패 | 로그 확인 |
| 🔴 Offline | 5분 이상 미응답 | 에이전트 상태 확인 |

### 로그 확인

```bash
# Docker
docker logs -f trivy-agent

# Systemd
journalctl -u trivy-agent -f
```

---

## 🗑️ 에이전트 관리

### 중복 에이전트 정리

에이전트가 재시작될 때 새 agent_id가 생성되어 중복 등록될 수 있습니다.

**웹 UI에서 정리:**
1. **에이전트 관리** 페이지 접속
2. 우측 상단 **🗑️ 중복 정리** 버튼 클릭
3. hostname 기준 중복된 에이전트 중 오래된 것 자동 삭제

**개별 삭제:**
- 각 에이전트 카드의 **🗑️** 버튼 클릭 (Admin만)

### API로 정리

```bash
# 중복 정리
curl -X POST -b "PHPSESSID=YOUR_SESSION" \
  "http://server:6987/api/agent_admin.php" \
  -d "action=cleanup_agents"

# 특정 에이전트 삭제
curl -X POST -b "PHPSESSID=YOUR_SESSION" \
  "http://server:6987/api/agent_admin.php" \
  -d "action=delete_agent&agent_id=OLD_AGENT_ID"
```

### AGENT_ID 고정하기

중복 방지를 위해 환경변수로 고정:

```bash
docker run -d \
  -e AGENT_ID=my-server-01 \
  ...
```

---

## 🔐 보안 권장사항

1. **토큰 관리**: `.env` 파일에 저장, 정기적 갱신
2. **HTTPS 사용**: 프로덕션에서는 TLS 필수
3. **IP 제한**: 알려진 에이전트 IP만 허용
4. **최소 권한**: 필요한 Collector만 활성화

```bash
# 보안 강화 실행 예시
docker run -d \
  --name trivy-agent \
  --read-only \
  --security-opt no-new-privileges \
  --cap-drop ALL \
  --cap-add NET_RAW \
  -v /var/run/docker.sock:/var/run/docker.sock:ro \
  -e CENTRAL_API_URL=https://secure-server.com/api/agent.php \
  -e AGENT_TOKEN=$(cat /run/secrets/agent_token) \
  trivy-agent:latest
```

